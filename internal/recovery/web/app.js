...
function renderMetrics() {
  ...
  text($("#metric-session-note"),`${number(state.dashboard.coverage.history_returned)} returned · ${number(counts.relationships)} linked · ${number(counts.reviewed_sessions)} reviewed`);
}

function renderCoverage() {
  const coverage=state.dashboard.coverage; const strip=$("#coverage-strip"); strip.replaceChildren();
  strip.append(fragment("Fleet revision",state.dashboard.revision||"unversioned"),fragment("Fleet read",formatTime(state.dashboard.read_timestamp)),fragment("Snapshot",`${number(coverage.fleet_snapshot_returned)} / ${number(coverage.fleet_limit)}`),fragment("Active overlay",coverage.active_task_limit?`${number(coverage.active_task_returned)} / ${number(coverage.active_task_limit)}`:"disabled"),fragment("Relationship coverage",coverage.fleet_worklinks_capped?`targeted · ${number(coverage.session_links_hydrated)} sessions · ${number(coverage.session_tasks_hydrated)} tasks`:"snapshot complete"),fragment("Reviewed history",number(state.dashboard.counts.reviewed_sessions)),fragment("History coverage",coverage.history_partial?"bounded":"complete response"),fragment("Generated",formatTime(state.dashboard.generated_at)));
}
...
function openSession(session,finding=null){if(!session)return;++state.detailEpoch;state.selectedTask="";state.selectedSession=session.session_id;const relation=state.dashboard?.relationships?.find(item=>item.session_id===session.session_id&&item.kind!=="suggested"),disposition=state.dashboard?.session_dispositions?.find(item=>item.session_id===session.session_id);text($("#drawer-eyebrow"),finding?.label||disposition?.label||session.chunk_type||"History evidence");text($("#drawer-title"),session.project_name||session.session_id);const body=$("#drawer-body");body.replaceChildren(detailSection("Session",detailGrid({ID:session.session_id,Project:session.project_name||"—",Path:session.project_path||"—",Machine:session.machine_id||"—",Recorded:formatTime(session.timestamp),Match:relation?`${relation.kind} · ${relation.score}`:disposition?`reviewed · ${disposition.label}`:"unbound"})));if(disposition)body.append(detailSection("Recovery disposition",detailGrid({Kind:disposition.label,Owner:disposition.task_id,Artifact:disposition.artifact_ref,Recorded:formatTime(disposition.created_at)})),detailSection("Disposition evidence",text(el("div","summary-block"),disposition.detail||"No note recorded.")));body.append(detailSection("History evidence",text(el("div","summary-block"),session.summary||"No content returned.")));setDrawerActions(relation?.task_id||"",session.session_id);openDrawer();}
